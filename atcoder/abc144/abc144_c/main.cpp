...
template <typename T> T distance_L1(T x1, T y1, T x2, T y2) {
  // put("(x1, y1, x2, y2): (" <<x1<< ", " <<y1<< ", " <<x2<< ", " <<y2<< ")");
  return (x1 - x2 < 0 ? x2 - x1 : x1 - x2) + (y1 - y2 < 0 ? y2 - y1 : y1 - y2);
}
...
int main() {
  ...
  vector<i64> df = get_divisors(N);

  i64 min_ = INFLL;
  for (i64 i = 0; i < df.size(); ++i) {
    i64 x = df[i];
    if (N % x != 0)
      continue;
    i64 y = N / x;
    chmin(min_, distance_L1((i64)1LL, (i64)1LL, x, y));
  }
