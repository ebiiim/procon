...
int main() {
  ...
  auto mA = min_element(A.begin(), A.end());
  size_t imA = distance(A.begin(), mA);

  auto mB = min_element(B.begin(), B.end());
  size_t imB = distance(B.begin(), mB);
  ...
  // 同じ2人、右をチェック
  sort(B.begin(), B.end());
  int right = max(vA, B[1]);

  // put("2lr: "<< do_two << " " << left << " " << right);

  put (min ( min (do_two, left), right));
