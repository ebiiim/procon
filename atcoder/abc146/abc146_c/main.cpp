...
int main() {
  ...
  auto d = [](i64 n) { return to_string(n).size(); };
  auto price = [&A, &B, &X, &d](i64 n) { return A * n + B * d(n); };

  i64 N = 1;

  i64 left = 1, right = 1000000000LL;
  i64 prev_left = -1, prev_right = -1;
  ...
  while (1) {
    ...
    if (prev_left == left && prev_right == right) {
      for (i64 cand = right; cand >= left; --cand) {
        if (price(cand) <= X) {
          put(cand);
          exit(0);
        }
        if (left == 1 && price(left) > X) {
          put(0);
          exit(0);
        }
      }
