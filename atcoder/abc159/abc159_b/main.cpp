...
int main() {
  ...
  bool b1 = is_palindrome(S);
  int N = S.size();
  // put("B1 " << S.substr(0, (N - 1) / 2));
  // put("B2 " << S.substr((N + 3) / 2 - 1, N - ((N + 3) / 2 - 1)));
  bool b2 = is_palindrome(S.substr(0, (N - 1) / 2));
  bool b3 = is_palindrome(S.substr((N + 3) / 2 - 1, N - ((N + 3) / 2 - 1)));
