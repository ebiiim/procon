...
int main() {
  ...
  // func
  i64 left= -10000000123;
  i64 right = 10000000123;
  i64 addv = 0;
  // i64 nowv = 0;
  rep(i, N) {
    // put("AT " << AT[i].first << "|" << AT[i].second);
    if (AT[i].second == 1){
      // put("ADD");
      addv += AT[i].first;
      // nowv += AT[i].first;
    ...
    if (AT[i].second == 2){
      // put("MAX");
      chmax(left, AT[i].first);
      chmax(right, AT[i].first);
    }
    if (AT[i].second == 3){
      // put("MIN");
      chmin(left, AT[i].first);
      chmin(right, AT[i].first);
    }
    // put(left << "|" << right << "|" << addv);
  }
